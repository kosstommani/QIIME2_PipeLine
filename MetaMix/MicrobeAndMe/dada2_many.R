...
parser$add_argument('--minOverlap', type='integer',
                    default=15,
                    help='기본값: 15.')
# parser$add_argument('--no_lenFilter',
#                     action='store_true',
#                     help='Assembled Read에 대한 Length Fitler를 적용하지 않음.')
# parser$add_argument('--lenFilterF', type='integer',
#                     default=262,
#                     help='기본값: 262. V3V4(Bakt_341F-805R)일 경우 262~462(300bpX2: 300~500).')
# parser$add_argument('--lenFilterR', type='integer',
#                     default=462,
#                     help='기본값: 462. V3V4(Bakt_341F-805R)일 경우 262~462(300bpX2: 300~500).')
...
### Set Path ###
order_number.dir <- file.path(args$analysis_path, args$Order_Number)
...
if (!dir.exists(rawdata_run.dir)) {
    ...
    message(args$fastq_path)
    quit('no', 1)
}

# 한 번에 여러 시료를 분석할 경우
if (is.null(args$sample_name)){
    run_samples.dir <- list.dirs(rawdata_run.dir, recursive=FALSE)
    samples.name <- basename(run_samples.dir)
    fileCat(infoText('Sample Name'), file=log.file, append=TRUE)
    fileCat('Count: ', length(samples.name), file=log.file, append=TRUE)
    fileCat('\n', file=log.file, append=TRUE)
    fileCat(samples.name, file=log.file, append=TRUE)
    fileCat('\n\n', file=log.file, append=TRUE)
} else {
...
if (is.null(args$sample_name)){
    # 한 번에 여러 시료를 분석할 경우
    dada2.dir <- file.path(analysis.dir, 'R_DADA2')
} else {
...
fileCat(rownames(filterAndTrim.out), sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
fileCat(filterAndTrim.out, sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
...
fileCat(rownames(denoise_track), sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
fileCat(denoise_track, sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
# Denoise Track 저장
...
fileCat(rownames(mergers_track), sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
fileCat(mergers_track, sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
# Mergers Track 저장
...
fileCat(rownames(dist_length), sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
fileCat(dist_length, sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
...
### Length Filter ###
#-------------------#
# if (args$no_lenFilter == TRUE) {
#     fileCat(red('Length Filter 미적용!!!\n'), file=log.file, append=TRUE)
#     seqtab.LenFilter <- seqtab
# } else {
#     fileCat('6_2) Legnth Filter\n', sep='', file=log.file, append=TRUE)
#     seqtab.LenFilter <- seqtab[, nchar(colnames(seqtab)) %in% args$lenFilterF:args$lenFilterR]
#     # ASVs 개수 출력
#     fileCat(infoText('시료개수\tASVs개수\n'), file=log.file, append=TRUE)
#     if (is.null(args$sample_name)) {
#         fileCat(dim(seqtab.LenFilter), sep='\t', file=log.file, append=TRUE)
#     } else {
#         fileCat('1', length(names(seqtab.LenFilter)), sep='\t', file=log.file, append=TRUE)
#     }
#     fileCat('\n', file=log.file, append=TRUE)
#     # 길이별 분포 출력
#     fileCat(infoText('길이별 분포\n'), file=log.file, append=TRUE)
#     dist_length.lenFilter <- table(nchar(getSequences(seqtab.LenFilter)))
#     fileCat(rownames(dist_length.lenFilter), sep=' ', file=log.file, append=TRUE)
#     fileCat('\n', file=log.file, append=TRUE)
#     fileCat(dist_length.lenFilter, sep=' ', file=log.file, append=TRUE)
#     fileCat('\n', file=log.file, append=TRUE)
#     # 길이별 분포 저장
#     lenFilter_length.out_file <- file.path(dada2.dir, 'dada2.len_filter.length')
#     write.table(dist_length.lenFilter, lenFilter_length.out_file, append=TRUE,
#                 sep='\t', row.names=TRUE, col.names=table.names, quote=FALSE)
#     fileCat(successText('Length Filter 완료\n'), file=log.file, append=TRUE)
#     # Track 데이터 생성
# }
# TODO: Length Filter 후 데이터 구조가 변경됨.
# seqtab.LenFilter <- seqtab[, nchar(colnames(seqtab)) %in% 400:430]
# > table(nchar(getSequences(seqtab)))
#
# 401 402 403 404 405 407 408 409 411 421 422 423 424 426 427 428
# 39 778  34 114  59   4   1   1   7  21 932 197  19  27 196 111
# > table(nchar(getSequences(seqtab.LenFilter)))
#
# 401 402 403 404 405 407 408 409 411 421 422 423 424 426 427 428
# 39 778  34 114  59   4   1   1   7  21 932 197  19  27 196 111
# > test_seqtab.noChimera <- removeBimeraDenovo(seqtab, multithread=TRUE, verbose=TRUE)
# Identified 2385 bimeras out of 2540 input sequences.
# > seqtab.noChimera <- removeBimeraDenovo(seqtab.LenFilter, multithread=TRUE, verbose=TRUE)
# Identified 2314 bimeras out of 2540 input sequences.
...
if (is.null(args$sample_name)) {
    ...
    fileCat('1', length(names(seqtab.noChimera)), sep='\t', file=log.file, append=TRUE)
}
fileCat('\n', file=log.file, append=TRUE)
...
fileCat(rownames(noChimera_track), sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
fileCat(noChimera_track, sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
...
fileCat(successText('seqtab_noChimera 저장 완료\n'))
...
if (length(samples.name) == 1) {
    ...
    track <- cbind(filterAndTrim.out,
                   ...
                   sapply(dada_R2, getN),
                   sapply(mergers, getN),
                   rowSums(seqtab.LenFilter),
                   rowSums(seqtab.noChimera))
}
colnames(track) <- c('Input', 'Filtered', 'Denoised_R1', 'Denoised_R2', 'Merged', 'Length_Filter', 'noChimera')
rownames(track) <- samples.name
# 출력 & Log
fileCat(infoText('DADA2 Summary\n'), file=log.file, append=TRUE)
fileCat(rownames(track), sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
fileCat(track, sep=' ', file=log.file, append=TRUE)
fileCat('\n', file=log.file, append=TRUE)
# 파일 저장
...
write.table(trans_seqtab.noChimera, dada2_output.tsv,
            sep='\t', row.names=TRUE, col.names=col.names, quote=FALSE)
