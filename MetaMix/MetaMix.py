...
@click.option('--queue', '-q',
              default=BLAST_QUEUE,
              show_default=True,
              type=click.Choice(['bi3m.q', 'meta.q']),
              help='작업을 진행할 Queue.')
...
def run_pipeline(kargs):
    ...
    run_cofi_alignment(kargs['order_number'], analysis_dir_name)
    alignment_end_time = time.time()

    # Phylogeny
    run_cofi_phylogeny(kargs['order_number'], analysis_dir_name)
    ...
    # BIOM
    run_cofi_biom(kargs['order_number'], analysis_dir_name)
    ...
    if kargs['no_theCups'] is True:
        ...
        run_thecups(kargs['theCups_cmd'], kargs['order_number'], analysis_dir_name)
        thecups_end_time = time.time()
    return {
...
def get_cofi_alignment_cmd(p_order_number, p_analysis_number):
    """
...
def run_cofi_alignment(p_order_number, p_analysis_number):
    """
    ...
    cofi_alignment_cmd = get_cofi_alignment_cmd(p_order_number, p_analysis_number)
...
def get_cofi_phylogeny_cmd(p_order_number, p_analysis_number):
    """
...
def run_cofi_phylogeny(p_order_number, p_analysis_number):
    """
    ...
    :return:
    """
    cofi_phylogeny_cmd = get_cofi_phylogeny_cmd(p_order_number, p_analysis_number)
...
def get_cofi_biom_cmd(p_order_number, p_analysis_number):
    global COFI
    cofi_cmd = \
        'python3 {cofi} ' \
        'BIOM ' \
...
def run_cofi_biom(p_order_number, p_analysis_number):
    cofi_biom_cmd = get_cofi_biom_cmd(p_order_number, p_analysis_number)
...
def get_thecups_cmd(p_thecups_cmd, p_order_number, p_anslysis_number):
    global THECUPS
    thecups_cmd = \
        f'python3 {THECUPS} ' \
        f'{p_thecups_cmd} ' \
...
def run_thecups(p_thecups_cmd, p_order_number, p_analsyis_number):
    thecups_cmd = get_thecups_cmd(p_thecups_cmd, p_order_number, p_analsyis_number)
